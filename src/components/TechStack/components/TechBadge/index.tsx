...
const TechBadge = ({ title, style }: TechBadgeProps) => {
  ...
  return (
    <div
      ...
      style={
        hover ? {borderColor: style.backgroundColor} : { borderColor: 'white'}
      }
      onMouseOver={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
    >
      <div className={`relative z-3 rounded-full py-2 font-bold px-3 flex items-center transition-color duration-100`} style={
        (!hover) ? {color: 'white'} : {color: style.color}
      }
        >
